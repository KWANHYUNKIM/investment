...
import { api, BlogPost, AdminUser, AdminStatus, VisitorStats, Curation } from "@/lib/api";

const GREEN = "#217346";

type Sub = "blog" | "curation" | "manage" | "stats";
      ...
      {post && (
        <div className="rounded-md border border-[#e5e5e5]">
          ...
          <div className="flex gap-1 border-b border-[#eee] px-3 py-1.5 text-xs">
            ...
            {post.tags.length > 0 && <span className="ml-auto self-center text-[11px] text-[#999]">태그: {post.tags.join(", ")}</span>}
          </div>
          <div className="max-h-[520px] overflow-auto p-4">
            {view === "preview" ? (
              <div className="blog-preview text-sm leading-relaxed [&_h2]:mb-2 [&_h2]:mt-3 [&_h2]:text-lg [&_h2]:font-bold [&_h3]:mb-1 [&_h3]:mt-3 [&_h3]:font-semibold [&_table]:my-2 [&_table]:w-full [&_td]:border [&_td]:border-[#ddd] [&_td]:px-2 [&_td]:py-1 [&_th]:border [&_th]:border-[#ddd] [&_th]:bg-[#f2f2f2] [&_th]:px-2 [&_th]:py-1 [&_p]:my-1.5 [&_blockquote]:border-l-4 [&_blockquote]:border-[#217346] [&_blockquote]:pl-3 [&_blockquote]:text-[#555] [&_ul]:my-1.5 [&_ul]:list-disc [&_ul]:pl-5"
                dangerouslySetInnerHTML={{ __html: post.html }} />
            ) : (
              <pre className="whitespace-pre-wrap break-words text-[12px] text-[#333]">{view === "markdown" ? post.markdown : post.html}</pre>
            )}
          </div>
        </div>
      )}
