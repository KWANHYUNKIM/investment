"""Income HTTP routes — thin transport layer.

Each handler only: reads/validates query/body params, resolves the
authenticated user, and delegates to the injected ``IncomeService``. No
business logic, no storage access. Paths, params, and docstrings are unchanged
...
from .deps import get_income_service
from .service import IncomeService

router = APIRouter(prefix="/api/data", tags=["income"])

Svc = Depends(get_income_service)


@router.get("/income/overview")
def income_overview(user: str = Depends(require_auth), svc: IncomeService = Svc):
    """소득 종합 — 급여·부업·투자수익 + 월 총소득 + 조언."""
    return svc.overview(user)


@router.get("/income/salary")
def income_salary_get(user: str = Depends(require_auth), svc: IncomeService = Svc):
    return svc.get_salary(user)
...
def income_salary_set(earnings: list[dict] = Body(...), deductions: list[dict] = Body(default=[]),
                      ...
                      svc: IncomeService = Svc):
    """지급/공제 항목별 급여 저장(월 기준). [{label, amount}]"""
    return svc.set_salary(user, earnings, deductions, memo)
...
def income_raise_sim(raise_pct: float = Query(default=0), raise_amount: float = Query(default=0),
                     ...
                     user: str = Depends(require_auth), svc: IncomeService = Svc):
    """급여 인상 시나리오 + 인상분 적립투자 복리 미래가치."""
    return svc.raise_sim(user, raise_pct, raise_amount, years, invest_ratio, annual_return)


@router.get("/income/side")
def income_side_list(month: str | None = Query(default=None), user: str = Depends(require_auth),
                     svc: IncomeService = Svc):
    return svc.list_side(user, month)
