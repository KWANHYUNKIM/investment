...
검사 대상
  ...
  · ``dart_full``       (§15.2) 전 항목 — 원재료 매입액·매출실적·영업부문·재고·원단위
  · ``integrity``       (§15.1) 진실성 스코어 X1~X35(DuckDB 필요)
...
DEFAULT = {
    # §15의 기준 문서. 원재료 매입액·단가·생산량이 모두 공시돼 **원단위 역산**이 도는 유일한 예다.
    "161890": "한국콜마(화장품ODM·§15 기준문서)",
    "004370": "농심(음식료·조별)",
...
def check_dart_full(tk: str, problems: list[str]) -> dict:
    """(§15.2) 전 항목 파서 — **합계행을 항목으로 세면 합이 정확히 두 배**가 된다.

    실제로 물렸던 것들이라 규칙으로 박아 둔다:
      · 부문매출 합이 연결매출의 1.8배를 넘으면 합계열(계·부문계)을 부문으로 센 것이다.
      · 매출실적 합이 음수면 매출에누리 차감행만 더한 것이다.
      · 레벨3 비중이 100%를 넘으면 '공정가치로 측정되지 않는 부채'까지 더한 것이다.
    """
    from app.data.fundamentals import dart_full as dfm
    d = dfm.full(tk)
    out = {"available": d.get("available"), "parsed": d.get("parsed") or [],
           "sections": len(d.get("sections_found") or []),
           "notes": len(d.get("notes_found") or []),
           "unit_consumption": len(d.get("unit_consumption") or [])}
    seg = d.get("segments")
    sm = d.get("sales_mix")
    if seg:
        out["segment_total_eok"] = round(seg["total_revenue_won"] / 1e8)
        if any(_BAD_NAME.match(r["name"]) for r in seg["rows"]):
            _fail(problems, tk, "부문명 이상 — 표 오독")
    if sm:
        tot = sm.get("total_by_year") or {}
        out["sales_total_eok"] = {k: round(v / 1e8) for k, v in tot.items()}
        if tot and min(tot.values()) < 0:
            _fail(problems, tk, "매출실적 합계 음수 — 차감행만 집계한 것")
    mp = d.get("materials_purchase")
    if mp:
        out["materials_eok"] = round(mp["total_won"] / 1e8)
        if mp["total_won"] <= 0:
            _fail(problems, tk, "원재료 매입액 합계 0 이하 — 단위 인식 실패")
    fv = (d.get("other_financial") or {}).get("fair_value") or {}
    if fv.get("level3_pct") is not None and fv["level3_pct"] > 100:
        _fail(problems, tk, f"레벨3 비중 {fv['level3_pct']}% — 비측정 부채 혼입")
    for u in d.get("unit_consumption") or []:
        if not (0 < u["u"] < 1e6):
            _fail(problems, tk, f"원단위 이상 {u['material']} {u['u']}{u['u_unit']}")
    return out


def check_integrity(tk: str, problems: list[str]) -> dict:
    """(§15.1) 스코어 엔진 — 두 절의 숫자가 맞물리는지. **두 배 차이는 파서 문제다.**"""
    from app.data.fundamentals import (dart_full as dfm, integrity as ig,
                                       labor_cost as lc, report_business as rb,
                                       report_notes as rn)
    d = dfm.full(tk)
    sep = d.get("separate") or {}
    try:
        r = ig.evaluate(tk, dfull=d, notes=rn.notes(tk), labor=lc.analyze(tk),
                        biz=rb.business(tk), separate=(sep.get(max(sep)) if sep else None))
    except Exception as e:
        return {"available": False, "reason": f"{type(e).__name__}: {str(e)[:60]}"}
    if not r.get("available"):
        return {"available": False, "reason": "재무제표 미적재(DB 잠금 포함)"}
    fails = [c for c in r["checks"] if c["status"] == "fail"]
    # 합계 이중계상은 대략 +100%로 나타난다 — 판정이 아니라 **파서 회귀** 신호다.
    for c in fails:
        if c["code"] in ("X11", "X12", "X13") and "+100." in (c["detail"] or ""):
            _fail(problems, tk, f"{c['code']} 정확히 2배 — 합계행 이중계상 의심: {c['detail'][:60]}")
    return {"available": True, "score": r["score_pct"], "coverage": r["coverage_pct"],
            "grade": r["grade"], "fails": [c["code"] for c in fails],
            "na": [c["code"] for c in r["checks"] if c["status"] == "na"]}
...
def main() -> int:
    ...
    for tk, label in targets.items():
        ...
        row["dart_full"] = check_dart_full(tk, problems)
        row["audit"] = {"skipped": True} if args.skip_audit else check_audit(tk, problems)
        row["integrity"] = {"skipped": True} if args.skip_audit else check_integrity(tk, problems)
        ...
        dfr = row["dart_full"]
        print(f"   전항목: 절 {dfr['sections']}개 · 주석 {dfr['notes']}개 · "
              f"{','.join(dfr['parsed']) or '없음'} · 원단위 {dfr['unit_consumption']}건")
        ir = row["integrity"]
        if ir.get("available"):
            print(f"   진실성: {ir['score']}% [{ir['grade']}] · 검증범위 {ir['coverage']}%"
                  f"{' · 불일치 ' + ','.join(ir['fails']) if ir['fails'] else ''}")
        elif not ir.get("skipped"):
            print(f"   진실성: 건너뜀({ir.get('reason')})")
